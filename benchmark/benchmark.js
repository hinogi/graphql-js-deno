...
if (require.main === module) {
  const { benchmarks, revisions } = getArguments(Deno.argv.slice(2));
  ...
  for (const benchmark of benchmarks) {
    ...
    for (let i = 0; i < benchmarkProjects.length; ++i) {
      ...
      try {
        ...
        Deno.stdout.write('  ' + cyan(i + 1) + ' tests completed.\u000D');
      } catch (error) {
...
function sampleModule(modulePath) {
  const sampleCode = `
    ...
    global.gc();
    Deno.nextTick(() => {
      const memBaseline = Deno.memoryUsage().heapUsed;
      const clocked = clock(module.count, module.measure);
      Deno.send({
        name: module.name,
        clocked: clocked / module.count,
        memUsed: (Deno.memoryUsage().heapUsed - memBaseline) / module.count,
      });
    });
    ...
    function clock(count, fn) {
      ...
      return Number(Deno.hrtime.bigint() - start);
    }
  `;
